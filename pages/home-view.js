import {
  ...
  const HomeView = ({ navigation }) => {
      ...
      if (user) {
        const fetchUserName = async () => {
          try {
            ...
            setIsLoading(false); // Moved inside the async function, ensuring it's called after the fetch is complete
          }
        };
  
        // Add a delay before fetching the user name
        setTimeout(() => {
          fetchUserName();
        }, 500);
      } else {
    ...
    }, [user]); // Added `user` as a dependency, since the effect relies on it
  ...
  const styles = StyleSheet.create({
    ...
    spinnerContainerStyle: {
      ...
      justifyContent: "center",
      alignItems: "center",
      backgroundColor: "#f4f4ff", // Optional: for semi-transparent background
